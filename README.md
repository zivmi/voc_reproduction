...
The ``benign overfitting" effect appears in Random Fourier Features model for timing the stock market, and it happens not only in terms of expected square loss but also in terms of financial performance metrics (SR, IR, alpha, etc). The trading strategy that invests in the market with a position that is proportional to the forecasted return (without limitations on leverage, no trading costs, etc.) has expected Sharpe Ratio which is monotonically increasing with the complexity of the model (i.e. the number of parameters of the model). The strategy is trend-following, rarely goes short, and can time recessions. Thus, the heuristics of using simple instead of complex benignly overfitting models (such as RF model) has no justification from the perspective preventing the risk of overfitting. **Note:** this does NOT imply that the random feature model will outperform a ridge regression. In fact, the random feature model in the context of forecasting returns (in the most cases) learns only a linear approximation of the true pricing function - and it does so very inefficiently.
